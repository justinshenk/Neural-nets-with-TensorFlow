...
def convLayer(inputs,
    ...
    kernels = tf.get_variable(
        "kernels", [kx, ky, channels, feature_maps],
        initializer=tf.truncated_normal_initializer(stddev=.1, seed=RAND_SEED))
    bias = tf.get_variable(
        "bias", [feature_maps], initializer=tf.constant_initializer(.1))
    convolution = tf.nn.conv2d(
        inputs, kernels, strides=strides, padding=padding)
    if act_fun:
...
def transposedConvLayer(inputs,
                        kx,
                        ky,
                        channels,
                        feature_maps,
    ...
    filter = tf.get_variable(
        "filter", [kx, ky, feature_maps, channels],
        initializer=tf.truncated_normal_initializer(stddev=.1, seed=RAND_SEED))
    bias = tf.get_variable(
        "bias", [feature_maps], initializer=tf.constant_initializer(.1))
    convolution = tf.nn.conv2d_transpose(
        inputs, filter, out_shape, strides=strides, padding=padding)
    if act_fun:
        return act_fun(convolution + bias)
    else:
        return convolution + bias
