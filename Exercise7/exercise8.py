...
def batch_normalize(batch):
    ...
    normalized = tf.nn.batch_normalization(batch, mean, variance, None, None,
                                           1e-10)  # what epsilon do I use?
    return normalized
...
if __name__ == "__main__":
    ...
    with tf.variable_scope("gen_conv1"):
        filter1 = tf.get_variable(
            ...
            initializer=tf.truncated_normal_initializer(
                stddev=.1, seed=RAND_SEED))
        bias1 = tf.get_variable(
            "bias", [256], initializer=tf.constant_initializer(.1))
        conv1 = tf.nn.conv2d_transpose(
            ff1_out,
            filter1,
            tf.pack([batch_size, 7, 7, 256]),
            strides=[1, 2, 2, 1],
            padding='SAME')
    ...
    with tf.variable_scope("gen_conv2"):
        ...
        bias2 = tf.get_variable(
            "bias", [128], initializer=tf.constant_initializer(.1))
        conv2 = tf.nn.conv2d_transpose(
            conv1_out,
            filter2,
            tf.pack([batch_size, 14, 14, 128]),
            strides=[1, 2, 2, 1],
            padding='SAME')
    ...
    with tf.variable_scope("gen_conv3"):
        ...
        bias3 = tf.get_variable(
            "bias", [1], initializer=tf.constant_initializer(.1))
        conv3 = tf.nn.conv2d_transpose(
            conv2_out,
            filter3,
            tf.pack([batch_size, 28, 28, 1]),
            strides=[1, 2, 2, 1],
            padding='SAME')
    ...
    targets_distinguisher = tf.constant(
        [0] * 32 + [1] * 32, shape=[64, 1], dtype=tf.float32)
    ...
    with tf.variable_scope("dist_ff1"):
        ...
        dist_ff1 = feedForwardLayer(
            tf.reshape(dist_conv4, [-1, np.prod(s[1:])]),
            np.prod(s[1:]),
            1,
            act_fun=None)  # do not apply activation function yet
    ...
    pbar = ProgressBar(
        widgets=widgets, maxval=n_epochs * data_train.shape[0] // 32).start()
    i = 0
