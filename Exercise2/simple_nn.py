...
def generate_data(sampleSize=30):
    ...
    targets = np.concatenate((np.full(sampleSize, -1, dtype=int),
                              np.ones(sampleSize)))
    return data, targets
...
def stochastic_gradient_descent(data,
    ...
    for k in range(iterations):
        ...
        weights += backpropagate(data[index], o0, o1, weights, targets[index],
                                 learning_rate)
    return weights
...
def print_data(index):
    print("data[{}]: target = {} \nnetwork says: {}".format(
        index, targets[index], classify(data[index], trained_weights)))


if __name__ == "__main__":
    data, targets = generate_data()
    data = pre_process_data(data)
    trained_weights = stochastic_gradient_descent(
        data, targets, iterations=1000)
    print("%% misclassified on training set: %f" % calc_misclassification(
        trained_weights, data, targets))
