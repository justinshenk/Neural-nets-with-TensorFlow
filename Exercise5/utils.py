...
def load_svhn_data(path='./iannwtf-svhn/',
                   train_data='trainData.pickle',
                   train_labels='trainLabels.pickle',
                   train_split=0.75,
                   test_split=0.10,
                   val_split=0.15,
                   flatten=False,
    ...
    assert abs(train_split + test_split +
               val_split) - 1 < 1e-4, "Split sizes don't sum to 1"
    ...
    with open(path + train_data, mode='rb') as data_file:
        with open(path + train_labels, mode='rb') as label_file:
            ...
            d_train, d_test, l_train, l_test = train_test_split(
                d, l, train_size=train_split, random_state=0)
            # split test set further
            d_test, d_val, l_test, l_val = train_test_split(
                d_test,
                l_test,
                train_size=test_split / (test_split + val_split),
                random_state=0)
            if flatten:
                new_shape = (-1, d_train.shape[1] * d_train.shape[2],
                             d_train.shape[3])
...
def plot_data():
    ...
    for l in set(l_train):
        ...
        print("Samples for label {}: {}".format(
            l, d_train_l.shape[0] / d_train.shape[0]))
