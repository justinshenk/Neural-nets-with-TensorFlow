...
if __name__ == "__main__":
    ...
    pool1 = tf.nn.max_pool(
        act1, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding="SAME")
    ...
    with tf.variable_scope("conv2"):
        act2 = convLayer(
            pool1, k2_sz, k2_sz, n_feat1, n_feat2, act_fun=act_fun)
    ...
    pool3 = tf.nn.max_pool(
        act3, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding="SAME")
    ...
    pool4 = tf.nn.max_pool(
        act4, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding="SAME")

    # first FF layer
    n1_hidden = 1024
    n_in = 4 * 4 * n_feat5 // 4
    with tf.variable_scope("ff1"):
        act4 = feedForwardLayer(
            tf.reshape(pool4, [-1, n_in]), n_in, n1_hidden, act_fun=act_fun)
